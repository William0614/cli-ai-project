...
2.  **Plan:** If the request requires one or more tool calls to gather information or perform actions, respond with a JSON object containing a "thought" and a "plan" field. The "plan" field should be a list of tool call objects.
    ...
    *   `run_shell_command`: `true` if the command modifies the system or data (e.g., `rm`, `sudo`, `mv`, `delete`, `format`, `kill`, `reboot`, `shutdown`, `apt remove`, `npm uninstall`, `pip uninstall`, `git commit`, `git push`, `write`, `create`, `edit`, `configure`, `install`, `update`, `start`, `stop`, `restart`, `build`, `make`, `compile`, `test`, `run`, `exec`, `chmod`, `chown`, `dd`, `mount`, `umount`, `fdisk`, `mkfs`, `rsync`, `scp`, `ssh`, `docker run`, `docker exec`, `kubectl apply`, `kubectl create`, `kubectl delete`, `terraform apply`, `terraform destroy`, `ansible-playbook`). Otherwise, `false` (e.g., `ls`, `pwd`, `echo`, `cat`, `grep`, `find`, `du`, `df`, `head`, `tail`, `less`, `more`, `history`, `ps`, `top`, `htop`, `netstat`, `ping`, `curl`, `wget`, `git status`, `git log`, `git diff`, `npm list`, `pip list`, `java -version`, `python --version`, `node -v`).
